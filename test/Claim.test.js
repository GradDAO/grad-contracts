...
describe("Claim Test", async () => {
  ...
  describe("Claim: SetTerm", async () => {
    ...
    it("SetTerms: Cannot change type of claimer", async () => {
      await claim.setTerm(carol.address, 5 * 1e3, gradDecimals.mul(1 * 1e5), 2);

      await expect(
        claim.setTerm(carol.address, 5 * 1e3, gradDecimals.mul(1 * 1e5), 0)
      ).to.be.revertedWith("Cannot change type of claimer");
    });
  ...
  describe("Claim: WalletChange", async () => {
    ...
    describe("WalletChange: pullWalletChange", async () => {
      ...
      it("pullWalletChange: Wallet already exists", async () => {
        await claim.setTerm(bob.address, 5 * 1e3, gradDecimals.mul(1 * 1e6), 1);
        await expect(
  ...
  describe("Claim: Buy Investor Allocation", async () => {
    it("Cannot buy without sale is started", async () => {
      await expect(
        claim
          .connect(investor)
          .buyInvestorsAllocation(investor.address, gradDecimals.mul(1000))
    ...
    it("Cannot buy more than allowed", async () => {
      ...
      await expect(
        claim
          .connect(investor)
          .buyInvestorsAllocation(investor.address, gradDecimals.mul(10000))
  ...
  describe("Claim: Withdraw", async () => {
    it("Withdraw: Ownable: caller is not the owner", async () => {
      await expect(
        claim
          .connect(bob)
          .withdraw(bob.address, dai.address, daiDecimals.mul(1 * 1e6))
